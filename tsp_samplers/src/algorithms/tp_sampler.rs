use std::{
    cmp::min,
    sync::{
        atomic::{AtomicU64, AtomicUsize, Ordering, AtomicU32},
        Mutex, MutexGuard,
    },
                ...
                s.spawn(move || {
                    for _ in 0..n_limit {
                        for _ in 0..n_att {
                            ...
                            let start = random_solution(n as u32, Some(rnglock.next_u64()), true);
                            drop(rnglock);
...
impl SamplingAlg for TPSampling {
